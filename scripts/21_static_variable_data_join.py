...
variable_data_dir = 'StateWiseVariableDataInterpolated'
...
for idx, state in enumerate(states):
    ...
    variable_data_file = os.listdir(f"{variable_data_dir}/{state}")[0]

    s_df = pd.read_excel(f"{static_data_dir}/{state}/{static_data_file}")
    v_df = pd.read_excel(f"{variable_data_dir}/{state}/{variable_data_file}")
    ...
    dest_dir = f"StateWise_Interpolated_Var_Stat_Case_Joined/{state}"
